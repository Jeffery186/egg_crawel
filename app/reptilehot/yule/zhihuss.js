...
const zhihuss = async (ctx) => {
    ...
    try {
        ...
        await Promise.all([
            ...
            page.goto("https://www.zhihu.com/hot?list=fashion", {
                waitUntil: "networkidle0",
            }),
            page.waitFor(2000),
        ]);

        let contextss = await page.content();
        let $ss = cheerio.load(contextss, {
            ignoreWhitespace: true,
            normalizeWhitespace: true,
        });
        let dataListss = [];
        const itemsss = $ss(".HotList-list .HotItem");

        itemsss.each((i, element) => {
            let url = $ss(element).find(".HotItem-content a").attr("href");

            let title = $ss(element).find(".HotItem-content a").attr("title");

            let heat = $ss(element)
                .find(".HotItem-content .HotItem-metrics")
                .text();

            let cover = $ss(element).find(".HotItem-img img").attr("src");
        ...
        ctx.app.saveCrawel(`知乎 时尚热榜`, dataListss);
