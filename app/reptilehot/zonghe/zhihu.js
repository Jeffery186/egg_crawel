...
const zhihu = async (ctx) => {
    ...
    try {
        ...
        let dataList = [];
        const items = $(".HotList-list .HotItem");

        items.each((i, element) => {
            let url = $(element).find(".HotItem-content a").attr("href");

            let title = $(element).find(".HotItem-content a").attr("title");

            let heat = $(element)
                .find(".HotItem-content .HotItem-metrics")
                .text();

            let cover = $(element).find(".HotItem-img img").attr("src");
        ...
        ctx.app.saveCrawel(`知乎 热榜`, dataList);

        //知乎热搜榜  AutoComplete-group .Menu-item  .SearchBar-topSearchItem  text
        ...
        let contextrs = await page.content();
        let $rs = cheerio.load(contextrs, {
            ignoreWhitespace: true,
            normalizeWhitespace: true,
        });
        let dataListrs = [];
        const itemsrs = $rs(".AutoComplete-group .Menu-item");

        itemsrs.each((i, element) => {
            let title = $rs(element).find(".SearchBar-topSearchItem").text();

            let url = `https://www.zhihu.com/search?type=content&q=${encodeURIComponent(
                title
            )}&utm_content=search_preset`;
        ...
        ctx.app.saveCrawel(`知乎 热搜榜`, dataListrs);
